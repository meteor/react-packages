...
const useFindSuspense = <T = any>(
  ...
  const entryKey = EJSON.stringify([collection._name, findArgs])
  const cachedEntry = cachedEntries.get(entryKey)

  if (cachedEntry != null) {
    if ('error' in cachedEntry || 'result' in cachedEntry) {
      setTimeout(() => cachedEntries.delete(entryKey), 0)
    }
  ...
  cachedEntries.set(entryKey, entry)
