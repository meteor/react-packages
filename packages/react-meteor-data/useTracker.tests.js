...
import useTracker, { useTrackerLegacy } from './useTracker';
...
if (Meteor.isClient) {
  ...
  const noDepsTester = async (test, mode = 'normal') => {
    ...
    test.equal(runCount, 4 * strictMul, 'After unmount, useTrackerLegacy should no longer be tracking');

    reactiveDict.destroy();
  };

  Tinytest.addAsync('useTrackerLegacy - Normal', async function (test, completed) {
    await noDepsTester(test);
    completed();
  });

  Tinytest.addAsync('useTrackerLegacy - in StrictMode', async function (test, completed) {
  ...
  Tinytest.addAsync('useTrackerLegacy - basic track', async function (test, completed) {
    var container = document.createElement("DIV");

    var x = new ReactiveVar('aaa');

    var Foo = () => {
      const data = useTrackerLegacy(() => {
        return {
          x: x.get()
        };
      });
  ...
  Tinytest.addAsync('useTrackerLegacy - render in autorun', async function (test, completed) {
    var container = document.createElement("DIV");
  ...
  Tinytest.addAsync('useTrackerLegacy - track based on props and state', async function (test, completed) {
    var container = document.createElement("DIV");
  ...
  testAsyncMulti('useTrackerLegacy - resubscribe', [
